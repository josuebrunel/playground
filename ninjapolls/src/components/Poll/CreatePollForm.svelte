<script>
    ...
    const ErrorQuestionLength = "Question must be at least 5 characters long";
    const ErrorEmptyAnswer = "Answer cannot be empty";
    const submitHandler = () => {
        valid = true;
        if (fields.question.trim().length < 5) {
            valid = false;
            errors.question = ErrorQuestionLength;
        } else {
            errors.question = "";
        }
        // validate answer A
        if (fields.answerA.trim().length < 1) {
            valid = false;
            errors.answerA = ErrorEmptyAnswer;
        } else {
            errors.answerA = "";
        }
        // validate answer B
        if (fields.answerB.trim().length < 1) {
            valid = false;
            errors.answerB = ErrorEmptyAnswer;
        } else {
            errors.answerB = "";
        }
        ...
        if (valid) {
            ...
            //save poll to store
            PollStore.update((storedPolls) => {
                return [poll, ...storedPolls];
            });
            dispatch("add");
        }
    };
